...
it("lets 2 users exchange stars", async () => {
  ...
  const owner1 = await instance.getStarOwner(starId1);
  const owner2 = await instance.getStarOwner(starId2);
...
it("lets a user transfer a star", async () => {
  ...
  const starOwner = await instance.getStarOwner(starId1);

  assert.equal(user2, starOwner);
});

it("tokenIdToStarInfo test", async () => {
  let tokenId = 99;
  const MOCK = "Mock name";
  let instance = await StarNotary.deployed();
  await instance.createStar(MOCK, tokenId, { from: accounts[0] });
  const name = await instance.tokenIdToStarInfo(tokenId);
